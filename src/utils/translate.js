export default class Translate {
  ...
  persistMissingTranslate(store, type, key, translate) {
    ...
    // ALEMAC // 10/03/2026 //
    // Bloqueia save fora da empresa ativa ou quando empresa ativa == people do usuario.
    const targetPeopleId = this.currentCompany?.id || this.defaultCompany?.id;
    const loggedUserPeopleId = this.getLoggedUserPeopleId();
    const appDomain =
      JSON.parse(localStorage.getItem("config") || "{}")?.["app-domain"] ||
      (typeof location !== "undefined" ? location.host : null);
    const enteredSave = this.canSaveMissingTranslate(targetPeopleId);

    if (enteredSave) {
      return this.translateActions.save({
        key,
        language: "/languages/1",
        people: "/people/" + targetPeopleId,
        store,
        translate: translate,
        type,
      }).then(() => {
        console.log('[TRANSLATE] translate save success', {
          key,
          store,
          type,
          targetPeopleId,
        ...
        console.log('[TRANSLATE] translate save error', {
  ...
  // Método para descobrir traduções de todos os stores com menos chamadas de API
  async discoveryAllAtOnce() {
    if (!this.translates || !this.translates[this.language]) {
      await this.fetchTranslates(null, this.defaultCompany);
      await this.fetchTranslates(null, this.currentCompany);
    }
    return this.translates;
  }
