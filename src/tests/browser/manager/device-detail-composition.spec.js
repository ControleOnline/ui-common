/**
 * Smoke browser: PDV detail and all tabs render with their complete dependencies.
...
const writeManifest = outputDir => {
  const manifest = {
    ...
    title: 'DeviceConfig PDV detail and tabs render without runtime errors',
    issue: 'ControleOnline/app-community#706',
...
const mockDevicesApi = async page => {
  ...
  await page.route(/^https:\/\/(?:api|s)\.controleonline\.com\//, async route => {
    ...
    if (pathname === 'device_configs' && method === 'GET') {
      return route.fulfill({
        status: 200,
        headers: jsonHeaders(),
        body: JSON.stringify(collection(deviceConfigs.filter(config => {
          const type = url.searchParams.get('type');
          const identifier = url.searchParams.get('device.device');
          return (!type || config.type === type) &&
            (!identifier || config.device.device === identifier);
        }))),
      });
    }
  ...
  await page.addInitScript(
    ({appVersion}) => {
      localStorage.setItem('session', JSON.stringify({id: 7, people: 7, active: 1, api_key: 'fixture-token-706'}));
...
test.describe('device-detail complete composition (#706)', () => {
  test.describe.configure({timeout: 60000});

  test('PDV detail and tabs open without runtime errors', async ({
    page,
  }, testInfo) => {
    ...
    expect(pageErrors.filter(error => /ReferenceError|is not defined|Maximum update depth/i.test(error))).toEqual([]);
    for (const [label, expected] of [
      [/^(Pdv ?Orders|Pedidos|Orders)$/i, /Pedidos do Device/i],
      [/^(Pdv ?Device|Dispositivo|Device)$/i, /Aviso Sonoro/i],
      [/^(Pdv ?Payments|Pagamentos|Payments)$/i, /Pagamentos do device/i],
      [/^(Pdv ?Movement|Movimento|Movement)$/i, /Produtos Vendidos/i],
    ]) {
      const control = page.getByText(label, {exact: true}).first();
      await control.click();
      await expect(page.getByText(expected).first()).toBeVisible();
      await writeEvidence(page, outputDir, `tab-${evidenceSteps.length}`, String(label));
    }
    expect(pageErrors.filter(error => /ReferenceError|is not defined|Maximum update depth/i.test(error))).toEqual([]);
    ...
    expect(manifest.steps.slice(0, 2).map(s => s.id)).toEqual([
      '01-lista-devices',
      '02-detalhe-pdv-510',
    ]);
  });
